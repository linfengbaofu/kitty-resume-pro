...
interface HeroVideoProps {
  ...
  thumbnailAlt?: string
  className?: string
  /** auto: native video for .mp4/.webm/.ogg, iframe for YouTube/Vimeo-style URLs. */
  player?: "auto" | "video" | "iframe"
}
...
function resolvePlayer(
  videoSrc: string,
  player: HeroVideoProps["player"],
): "video" | "iframe" {
  if (player === "video" || player === "iframe") return player
  if (/youtube\.com|youtu\.be|vimeo\.com/i.test(videoSrc)) return "iframe"
  if (/\.(mp4|webm|ogg)(\?|#|$)/i.test(videoSrc)) return "video"
  return "iframe"
}
...
export function HeroVideoDialog({
  ...
  const resolvedPlayer = resolvePlayer(videoSrc, player)
    ...
    <div className={cn("relative h-full w-full", className)}>
      ...
      >
        <img
          ...
          width={1920}
          height={1080}
          className="h-full w-full rounded-md border object-cover shadow-lg transition-all duration-200 ease-out group-hover:brightness-[0.8]"
        />
